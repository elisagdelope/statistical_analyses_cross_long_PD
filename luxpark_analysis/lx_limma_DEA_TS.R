# Title: lx_limma_DEA_TS.R
# Authorship: Elisa Gomez de Lope, Contact details: elisa.gomezdelope@uni.lu
# Info: This script performs DA on metabolomics data via limma on cosecutive timepoints  (t1 vs t2; t2 vs t3; t3 vs t4) for visit with Age & Gender as covariates separately for PD class.
# Usage: Rscript lx_limma_DEA_TS.R  
...
visits <- sort(unique(metab$VISIT))
...
for (v in seq(1:length(v1))) {
  ...
  PHENO[[paste0(v1[v],v2[v])]] <- pheno %>%
    dplyr::filter(VISIT %in% c(v1[v], v2[v]))
  
  METAB[[paste0(v1[v],v2[v])]] <- metab %>%
    dplyr::filter(VISIT %in% c(v1[v], v2[v])) %>%
...
for (v in seq(1:length(v1))) {
  ...
  METAB_class[[paste0(v1[v],v2[v])]] <- METAB[[paste0(v1[v],v2[v])]] %>%
    dplyr::filter(get("SAMPLE_ID") %in% class.flt) 
  
  if (nrow(METAB_class[[paste0(v1[v],v2[v])]]) > 2) {
    # Pre-processing ---------------------------------------------------------------
    
    # remove Zero variance (only 0-variance!)
    ...
    print(lm_summary)
    DEAS[[paste0(v1[v],v2[v])]] <- topTable(ebayes, coef=1, number = nrow(ebayes), sort.by = "P")
    DEAS[[paste0(v1[v],v2[v])]] <- as.data.frame(DEAS[[paste0(v1[v],v2[v])]]) %>%
      rownames_to_column(var = var_id)
    print(head(DEAS[[paste0(v1[v],v2[v])]]))
    
    # output results
    readr::write_tsv(DEAS[[paste0(v1[v],v2[v])]], file = file.path(OUT_DIR, paste0("lt_limma_", v1[v], v2[v], "_", class, ".tsv")))
...
for (t in names(DEAS)) {
  DEAS[[t]] <- DEAS[[t]] %>% 
    ...
    as.data.frame()
  flt <- c(flt, DEAS[[t]][is.na(DEAS[[t]]$adj.P.Val),][[var_id]]) # append all metabolites with NAs in padj variable
}
flt <- unique(flt)
DEAS[[1]] <- DEAS[[1]] %>% 
...
DEA_full <- Reduce(function(...) inner_join(..., by = var_id, suffix = c("_1", "_2")), DEAS) %>%
...
# output results ---------------------------------------------------------------
readr::write_tsv(DEA_consistent, file = file.path(OUT_DIR, "lt_consistent_trend_TS.tsv"))
