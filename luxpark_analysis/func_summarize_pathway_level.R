...
#      print(paste(names(gsets[j]), "did not pass minsize threshold; # matching genes:", length(notna)))
    ...
    if(type == "mean") {
      ...
      rem = which(apply(curpathmat, 1, var)==0) # variance for each row (for each feature, variance on all samples) => which features have variance 0 (e.g. same value for all samples)
      curpathmatfilt = curpathmat
      if(length(rem)) {
        curpathmatfilt = curpathmat[-rem,] # filter out variance 0 features
      }
      if(length(curpathmatfilt))
      {
        pca    <- prcomp(t(curpathmatfilt), retx=T, scale=T) # scaled pca 
        scores <- pca$x[,1] # value of rotated samples data in PC1
        meanpathvec = scores
      } else {
